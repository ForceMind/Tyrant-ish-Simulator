...
- 官网已作为默认入口，首屏不展示开发提示，只展示玩家可理解的模式入口、卖点和王朝预览。
- 快速版已按“人生模拟器式开局”拆成参数选择、关键节点推演、快速结局三段。
- 移动端快速版参数页使用真实分页，一屏只显示一组参数，底部保留上一组、开始、下一组按钮。
- 完整模式仍保留横向卡片切换、移动端底部数值条和 PC 分栏布局，二者样式通过断点隔离。
- 浏览器审核会覆盖官网、快速版、完整模式、回应页、档案页和长局自动游玩。
